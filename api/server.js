  ...
  try{
    switch(action){
      case 'joinSession': {
        ...
        const playerName = sanitizeName(context.playerName);
        const buildKey = sanitizeName(context.buildKey || context.playerName);
        const payload = [{
        ...
        if(buildKey){
          await supabaseMutate(`${encodeURIComponent(SUPABASE_BUILD_CARDS_TABLE)}`, {
            body: [{
              player_key: buildKey,
              class: sanitizeOptional(context.build?.class) || null,
              university: sanitizeOptional(context.build?.university) || null
            }],
