...
<section class="comment-board" id="commentBoard">
  ...
  `,Fe(e),He(e),e}function Je(){let e=document.createElement("div");e.className="panel",e.innerHTML=`
    <h2>Availability</h2>
        ...
        <div class="section-body">
          <div class="grid cols-2">
            <div>
              ...
              <select id="job"><option value="">\u2014 None \u2014</option>${I.jobs.map(m=>`<option value="${m.key}">${m.name}</option>`).join("")}</select>
            </div>
            <div>
  ...
  `;let t=e.querySelector("#join_list");return ke(t,{readOnly:B}),t.addEventListener("click",a=>{var i;let s=a.target.closest("button.primary[data-id]");if(s){if(B){alert(M);return}let c=s.getAttribute("data-id"),o=(b.sessions||[]).find(w=>w.id===c),u=(b.data.core.name||"").trim();if(!u){alert("Give your character a name (Core 5e).");return}if(ve.hardNo.includes(u)){alert(`${u} is marked as not playing.`);return}if((ve.blockedDates[u]||[]).includes(o.date)){alert(`${u} isn't available for ${o.date}.`);return}let h=Array.isArray(o.players)?o.players:[];if(h.some(w=>F(w&&w.character)===F(u))){alert("This character is already in that session.");return}if(h.some(w=>P(w&&w.key)===J)){alert("Your access code already has a seat in this session.");return}if(h.length>=o.capacity){alert("That session is full.");return}let S=((i=I.universities.find(w=>w.key===b.data.university.key))==null?void 0:i.name)||"",y=Ue();if(S)for(let w of h){let m=w&&w.key?y[w.key]:null;if(m&&m.university===S){alert(`Another ${S} student is already in this session. Choose a different session or college.`);return}}let E=b.data.core.class||"",q=z.getRosterEntry();ne.joinSession(c,{name:u,characterName:u,playerKey:J,playerName:q==null?void 0:q.name,build:{class:E,university:S}}).then(()=>{ke(t,{readOnly:B}),alert(`Added ${u} to ${o.title}.`)}).catch(w=>{alert(`Unable to join ${o.title}: ${w&&w.message?w.message:"Request failed."}`)});return}let r=a.target.closest("button[data-ics]");if(r){let c=r.getAttribute("data-ics"),o=b.sessions.find(u=>u.id===c);Me(o)}}),e.querySelector("#back_join").onclick=()=>{W(j.findIndex(a=>a.key==="builder"))},e.querySelector("#to_summary").onclick=()=>{W(j.findIndex(a=>a.key==="summary"))},e}function Qe(){var o;let e=document.createElement("div");e.className="panel";let t=b.data,a=I.universities.find(u=>u.key===t.university.key),s=Object.entries(t.core.abilities||{}).map(([u,h])=>`<span class="tag">${u}: ${h}</span>`).join(" "),r=(t.extras.clubs||[]).map(u=>{var h;return(h=I.extracurriculars.find(S=>S.key===u))==null?void 0:h.name}).filter(Boolean),i=((o=I.jobs.find(u=>u.key===t.extras.job))==null?void 0:o.name)||"\u2014";e.innerHTML=`
      <h2>Summary</h2>
      <div class="grid cols-2">
        <div class="card">
    ...
    `,e.querySelector("#back_s").onclick=()=>{W(j.findIndex(u=>u.key==="builder"))},e.querySelector("#save_s").onclick=()=>b.save(),e.querySelector("#export_s").onclick=()=>b.export(),e.querySelector("#pdf_s").onclick=()=>window.print();let c=e.querySelector("#publish_roster");return c&&(B?(c.disabled=!0,c.title=M,c.textContent="Roster editing disabled for guests"):c.onclick=async()=>{var w,m,f,A,x;let u=F((m=(w=b.data)==null?void 0:w.core)==null?void 0:m.name);if(!u){alert("Give your character a name in Core Setup first.");return}let h=P(u),S=[];(A=(f=b.data)==null?void 0:f.core)!=null&&A.class&&S.push(b.data.core.class);let y=a?a.name:((x=I.universities.find(N=>{var _,n;return N.key===((n=(_=b.data)==null?void 0:_.university)==null?void 0:n.key)}))==null?void 0:x.name)||"";y&&S.push(y);let E=S.join(" \u2022 "),q="Interested";if(de(h)){let _=U().find(n=>n.key===h)||{name:u,custom:!1};try{await ue(h,_,q,E),alert(`${u} is already on the roster. Updated their status and notes.`)}catch(n){alert(`Failed to update roster entry: ${n&&n.message?n.message:"Request failed."}`)}}else{let N=await xe(u,q,E);if(!N.ok){alert(N.msg);return}alert(`${u} added to the availability roster.`)}}),e}function ce(){try{Ie(),Ne(),qe()}catch(e){Te([`Render failed: ${String(e&&e.message||e)}`]),console.error(e)}}function qe(){let e=document.getElementById("btnSave");e&&(e.onclick=()=>b.save());let t=document.getElementById("btnLoad");t&&(t.onclick=()=>b.load());let a=document.getElementById("btnExport");a&&(a.onclick=()=>b.export());let s=document.getElementById("btnClear");s&&(s.onclick=()=>{confirm("Clear all local data for this app?")&&(re.clear(),localStorage.removeItem("oracleOfflineState"),alert("Local data cleared. Reloading\u2026"),location.reload())})}(async function(){let e=Le();if(e.length){Te(e),qe();return}try{await D.refresh()}catch(t){console.warn("Initial sync failed",t),ee.setError("Unable to reach the shared datastore. Showing cached data if available.")}ce()})();})();
